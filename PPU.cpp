...
void PPU::LCD::update_screen() {
    ...
    if (power_) [[likely]] {
        SDL_UpdateTexture(pixel_matrix_texture, nullptr, pixel_buffer.data(), DISPLAY_WIDTH * sizeof(uint8_t) * 4);
    } else {
        SDL_UpdateTexture(pixel_matrix_texture, nullptr, std::vector<uint8_t>(255).data(), DISPLAY_WIDTH * sizeof(uint8_t) * 4);
    }
    SDL_RenderCopy(renderer, pixel_matrix_texture, nullptr, nullptr);
...
void PPU::pixel_transfer_tick()
{
    ...
    if (BG_FIFO.empty() or pixel_fifo_stopped) return;
